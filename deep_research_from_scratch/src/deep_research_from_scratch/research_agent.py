...
model=ChatNVIDIA(NVIDIA_API_KEY=os.getenv('NVIDIA_API_KEY'),model='openai/gpt-oss-120b')
...
# compress_model=ChatGroq(groq_api_key=GROQ_API_KEY,model='openai/gpt-oss-120b',max_tokens=32000)
compress_model=ChatNVIDIA(NVIDIA_API_KEY=os.getenv('NVIDIA_API_KEY'),model='qwen/qwen3-next-80b-a3b-thinking',max_tokens=32000)
...
def tool_node(state: ResearcherState):
    ...
    return {"researcher_messages": tool_outputs,"tool_call_iterations":state['tool_call_iterations']+1}
...
def compress_research(state: ResearcherState) -> dict:
    ...
    return {
        "compressed_research": str(response.additional_kwargs['reasoning_content']),
        "raw_notes": ["\n".join(raw_notes)]
    }
...
def should_continue(state: ResearcherState) -> Literal["tool_node", "compress_research"]:
    ...
    tool_iterations=state.get('tool_call_iterations',0)
    if tool_iterations>max_tool_limit:
