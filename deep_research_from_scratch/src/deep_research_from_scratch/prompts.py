...
Respond in valid JSON format with these exact key:
...
DO NOT return `key_excerpts` and `summary` as a list of  string. 
...
Respond in valid JSON format with these exact keys:
   "summary": "<Concise summary of the webpage content>",
   "key_excerpts": "<Important quotes and excerpts from the content>"
...
**Important Reminders:**
- Remember to use think tool before and after ConductResearch.
...
6. Respond in valid JSON format with these exact keys:
   "criteria_text": "<The specific success criteria being evaluated (e.g., 'Current age is 25', 'Monthly rent below 7k')>",
   "reasoning": "<Detailed explanation of why this criteria is or isn't captured in the research brief, including specific evidence from the brief>",
   "is_captured": "<Whether this specific criteria is adequately captured in the research brief (True) or missing/inadequately addressed (False)>"
</output_instructions>"""
...
<output_instructions>
1.Carefully scan the brief for any details not explicitly provided by the user. Be strict - when in doubt about whether something was user-specified, lean toward FAIL.
2. Respond in valid JSON format with these exact keys:
   "no_assumptions": "<Whether the research brief avoids making unwarranted assumptions. True if the brief only includes information explicitly provided by the user, False if it makes assumptions beyond what was stated.>",
   "reasoning": "<Detailed explanation of the evaluation decision, including specific examples of any assumptions found or confirmation that no assumptions were made beyond the user's explicit statements.>"
</output_instructions>"""
